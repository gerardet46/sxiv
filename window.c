...
{
	win_env_t *e;
	const char *bg, *fg, *f, *bg_bar, *fg_bar;
	//char *res_man;
	//XrmDatabase db;
	...
	win_alloc_color(e, bg_bar, &win->bg_bar);
	win_alloc_color(e, fg_bar, &win->fg_bar);
...
void win_draw_bar(win_t *win)
{
	...
	XSetForeground(e->dpy, gc, win->bg_bar.pixel);
	XFillRectangle(e->dpy, win->buf.pm, gc, 0, win->h, win->w, win->bar.h);

	XSetForeground(e->dpy, gc, win->fg_bar.pixel);
	XSetBackground(e->dpy, gc, win->bg_bar.pixel);
	...
	if ((len = strlen(r->buf)) > 0) {
		...
		w -= tw;
		win_draw_text(win, d, &win->fg_bar, x, y, r->buf, len, tw);
	}
	if ((len = strlen(l->buf)) > 0) {
		x = H_TEXT_PAD;
		w -= 2 * H_TEXT_PAD; /* gap between left and right parts */
		win_draw_text(win, d, &win->fg_bar, x, y, l->buf, len, w);
	}
	XftDrawDestroy(d);
}
